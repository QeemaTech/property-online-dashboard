...
export default function CrudListPage({ title, queryKey, apiFn, columns, createPath, createLabel, searchPlaceholder, extraParams = {}, actions: extraActions }) {
  ...
  const defaultActions = (row) => (
    <div className="flex items-center gap-1">
      ...
      <button
        type="button"
        onClick={() => navigate(`${createPath?.replace('/create', '')}/${row.id}/edit`)}
  ...
  return (
    <div>
      <PageHeader title={title} createLabel={createLabel} createPath={createPath} />
      <DataTable
        columns={columns}
        data={data?.data}
        meta={data?.meta}
